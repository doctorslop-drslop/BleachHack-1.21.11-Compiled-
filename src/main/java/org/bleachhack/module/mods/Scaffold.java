...
public class Scaffold extends Module {
	...
	public Scaffold() {
		super("Scaffold", KEY_UNBOUND, ModuleCategory.WORLD, "Places blocks under you.",
				...
				new SettingSlider("BPT", 1, 40, 5, 0).withDesc("Blocks Per Tick, how many blocks to place per tick."),
				new SettingSlider("Range", 0, 3.0, 1.0, 1).withDesc("How far to place ahead of you in Normal mode."),
				...
				new SettingToggle("AirPlace", true).withDesc("Places blocks in the air without support blocks."),
				new SettingToggle("SafeWalk", true).withDesc("Prevents you from walking of edges when scaffold is on."),
				new SettingToggle("NoSwing", true).withDesc("Doesn't swing your hand clientside."),
	...
	public void onTick(EventTick event) {
		...
		if (getSetting(6).asToggle().getState()
			...
			if (!getSetting(6).asToggle().getChild(0).asToggle().getState()) {
				...
				} else if (mc.player.getVelocity().y < 0.15) {
